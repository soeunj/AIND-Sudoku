...
def diagonals(row, col):
    ...
    diag2 = []
    for i in range(9):
        diag1.append(row[i] + col[i])
        diag2.append(row[i] + col[len(col) - 1 - i])
    new_diagonal.append(diag1)
    new_diagonal.append(diag2)
    return new_diagonal
...
def naked_twins(values):
    ...
    for box in twins:
        for unit in units[box]:
            twin_box = [twin_box for twin_box in unit if values[twin_box] == values[box]]
            # Eliminate the naked twins as possibilities for their peers
            if len(twin_box) == 2:
                for num in values[box]:
                    for peer in unit:
                        if peer not in twin_box and len(values[peer]) > 1:
...
def only_choice(values):
    ...
    for unit in unitlist:
        for num in '123456789':
            onlychoice = [ box for box in unit if num in values[box]]
            if len(onlychoice) == 1:
                values[onlychoice[0]] = num
    return values
